...
global_step = -1
def train_epoch(epoch, model, model_d, optimizer, optimizer_d, train_loader, writer, args, stock2concept_matrix = None):
    ...
    for _, slc in tqdm(train_loader.iter_batch(), total=train_loader.batch_length):
        global_step += 1
        _, slc2 = next(train_loader.iter_batch())
        feature, label, market_value , stock_index, _ = train_loader.get(slc)
        feature2, _, _ , _, _ = train_loader.get(slc2)
        
        loss_vae = 0.
        pred, loss_vae,z = model(feature, stock2concept_matrix[stock_index], market_value, fVAE=True)
        loss_pred = loss_fn(pred, label, args)
        D_z = model_d(z)
        loss_vae_tc = (D_z[:, :1] - D_z[:, 1:]).mean()
        loss = loss_pred+(loss_vae[0]+loss_vae[1]+loss_vae[2])+ loss_vae_tc*6*0.1
        ...
        z_pperm = model(feature2, None, None, no_dec=True)
        D_z_pperm = model_d(z_pperm).detach()
        
        ones = torch.ones(D_z_pperm.size()[0], dtype=torch.long, device=device)
        zeros = torch.zeros(D_z.size()[0], dtype=torch.long, device=device)
        
        loss_D_tc = (0.5*(F.cross_entropy(D_z, zeros) + F.cross_entropy(D_z_pperm, ones)))*0.1

        optimizer_d.zero_grad()
        loss_D_tc.backward(inputs=list(model_d.parameters()))
        optimizer_d.step()
...
def main(args):
    ...
    if args.K:
      model_name = 'OursHIST_fVAE'
    else:
      model_name = 'OursLR_fVAE'
    ...
    for times in range(args.repeat):
        ...
        model.to(device)
        model_d = Discriminator(args.hidden_size).to(device)
        ...
        for epoch in range(args.n_epochs):
            ...
            pprint('training...')
            train_epoch(epoch, model, model_d, optimizer, optimizer_d, train_loader, writer, args, stock2concept_matrix)
